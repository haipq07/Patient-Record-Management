...
class BinarySearchTree:
    ...
    def inorder_traversal(self, node):# Time complexity: O(n)
        nodes = []
        if node:
            nodes.extend(self.inorder_traversal(node.left))#visit left subtree
            nodes.append(node) #visit current node
            nodes.extend(self.inorder_traversal(node.right))#visit right subtree
        return nodes

    def preorder_traversal(self, node):# Time complexity: O(n)
        elements = []
        if node:
            elements.append(node)  #visit current node
            elements.extend(self.preorder_traversal(node.left))#visit left subtree
            elements.extend(self.preorder_traversal(node.right))#visit right subtree
        return elements
    def postorder_traversal(self, node):# Time complexity: O(n)
        elements = []
        if node:
            elements.extend(self.postorder_traversal(node.left))#visit left subtree
            elements.extend(self.postorder_traversal(node.right))#visit right subtree
            elements.append(node)  #visit current node
        return elements
